		...
		try {
			...
			if (cell != null) {
				 ...
				 if (cell.getCellType() == Cell.CELL_TYPE_NUMERIC) {
					...
					if (val == null || "null".equals(val) || val.toString().length() <= 2) {
						return val;
					}

					if (val.toString().substring(0, 2).matches("[0-9][.]")) {
					...
					try {
						if (valType == String.class) {
							...
							try {
								...
								if (StringUtils.endsWith(valStr, ".0")) {
									val = StringUtils.substringBefore(valStr, ".0");
								} else {
									val = String.valueOf(valStr.toString());
								}
							} catch (Exception ex) {
