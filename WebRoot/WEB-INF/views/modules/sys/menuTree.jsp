...
<body> --%>
	<div class="accordion" id="menu-${param.parentId}"><c:set var="menuList" value="${fns:getMenuList()}"/><c:set var="firstMenu" value="true"/>
		<c:forEach items="${menuList}" var="menu" varStatus="idxStatus"><c:if test="${menu.parent.id eq (not empty param.parentId ? param.parentId:1)&& menu.childcount >0 &&menu.isShow eq '1'}">
		<div class="accordion-group">
			...
			<c:if test="${menu.parent.id eq (not empty param.parentId ? param.parentId:1)&& menu.childcount ==0 &&menu.isShow eq '1'}">
		     		  <div  class="accordion-body collapse in">
					  <div class="accordion-inner">
						<ul class="nav nav-list" >
							<li><a data-href=".menu3-${menu.id}" href="${fn:indexOf(menu.href, '://') eq -1 ? ctx : ''}${not empty menu.href ? menu.href : '/404'}" target="${not empty menu.target ? menu.target : 'mainFrame'}" ><i class="icon-${not empty menu.icon ? menu.icon : 'circle-arrow-right'}"></i>&nbsp;${menu.name}</a>
							 </li>
						</ul>
					  </div>
				  </div>
