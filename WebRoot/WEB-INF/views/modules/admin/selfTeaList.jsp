...
<script type="text/javascript">
	$(document).ready(function() {
		$("#btnExport").click(function() {
			top.$.jBox.confirm("确认要导出老师数据吗？", "系统提示", function(v, h, f) {
				if (v == "ok") {
					$("#searchForm").attr("action", "${ctx}/tea/selfExport");
					$("#searchForm").submit();
				}
			}, {
				buttonsFocus : 1
		...
		$("#addry").click(function() {
			...
			window.location = "${ctx}/tea/selfForm";
		});
			...
			swal({
				...
				$.ajax({
					...
					success : function(data) {
						toastr.success('停用成功');
						location = "${ctx}/tea/selfList?repage";
	...
	function page(n, s) {
		...
		if (s)
			$("#pageSize").val(s);
		$("#searchForm").attr("action", "${ctx}/tea/selfList");
		$("#searchForm").submit();
		return false;
	}
</script>
...
<body>
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/tea/selfImport" method="post"
			...
			<br /> <input id="uploadFile" name="file" type="file"
				...
				href="${ctx}/tea/selfImport/template">下载模板</a>
		</form>
	</div>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/tea/selfList">老师列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="zxsTea"
		action="${ctx}/tea/selfList" method="post"
		...
		<ul class="ul-form">
			...
			<li class="btns"><input id="btnSubmit" class="btn btn-primary"
				...
				value="导出" /> <shiro:hasPermission name="tea:edit:view">
		...
		<tbody>
			<c:forEach items="${page.list}" var="zxsTea" varStatus="ry">
				<tr>
					...
					<td><a href="${ctx}/tea/selfForm?id=${zxsTea.id}">${zxsTea.teaNo}</a>
					</td>
					...
					<shiro:hasPermission name="tea:edit:view">
						<td><a href="${ctx}/tea/selfForm?id=${zxsTea.id}">修改</a> <a
							href="${ctx}/tea/selfDelete?id=${zxsTea.id}"
							onclick="return confirmx('确认要删除该老师吗？', this.href)">删除</a> <c:choose>
								<c:when test="${zxsTea.delFlag==0}">

									<a href="${ctx}/tea/selfStop?del=2&id=${zxsTea.id}"
										onclick="return confirmx('确认要停用该老师吗？', this.href)">停用</a>
								</c:when>
								<c:when test="${zxsTea.delFlag==2}">
									<a href="${ctx}/tea/selfStop?del=0&id=${zxsTea.id}"
