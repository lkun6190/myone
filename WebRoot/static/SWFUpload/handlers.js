...
function uploadProgress(file, bytesLoaded) {
	...
	try {
		...
		if (percent === 100) {
			...
			if(percent <= 100)
				$('#progress_' +file.id ).attr('data-transitiongoal', percent).progressbar({display_text: 'center'});
...
function uploadSuccess(file, serverData) {
	try {
		var progress = new FileProgress(file,  this.customSettings.upload_target);
        var parsedJson = jQuery.parseJSON(serverData);
        addFileInfo(file.id,"文件上传完成", parsedJson); 
         
		if(this.customSettings.upload_complete_callback!=null) {
			this.customSettings.upload_complete_callback(file, serverData);
		}
...
function addFileInfo(fileId,message, parsedJson){
	var row = document.getElementById(fileId);
	row.cells[2].innerHTML = "<font color='green'>"+message+"</font>" +"<input type='hidden' id='fileId" + fileId + "'  value='" + parsedJson.id +"' />";
    row.cells[3].innerHTML = "<font color='green'>"+parsedJson.user+"</font>" ;
}
function addReadyFileInfo(fileid,fileName,message,status){
	...
	col4.innerHTML = "<div class=\"progress\" width='250'> "
	+ " <div class=\"bar\"  id=\"progress_" + fileid +"\" role=\"progressbar\" data-transitiongoal=\"0\"></div> "
	+"  </div>  ";
...
function deleteFile(fileId){
	...
	var infoTable = document.getElementById("infoTable");
	var row = document.getElementById(fileId);
    var serverId = $("#fileId" + fileId).val();
    if(serverId !=undefined ){
        $.ajax({
            url: ctx + "/ts/tsTsdjb/deletefile",
            data:{"id": serverId},
            type:"get",
            dataType:"json",
